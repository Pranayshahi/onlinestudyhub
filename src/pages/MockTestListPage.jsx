...
export default function MockTestListPage() {
  ...
  return (
    <div style={{ background: '#f9fafb', minHeight: '100vh' }}>
      <SEO
        title={`${examLabel} Free Mock Tests 2025 — Timed Full Practice Tests`}
        description={`Attempt ${tests.length} free ${examLabel} mock tests with real exam timer. Check your score, review answers, and track your preparation progress for ${examLabel} 2025.`}
        path={`/exam/${examId}/mock-test`}
        breadcrumbs={[
          { name: examLabel, url: `/exam/${examId}` },
          { name: 'Mock Tests', url: `/exam/${examId}/mock-test` },
        ]}
      />
      {/* Header */}
      <div style={{ background: `linear-gradient(135deg, #1e1b4b 0%, ${exam.color} 100%)`, color: '#fff', padding: '2.5rem 0 2rem' }}>
        <div className="container">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem' }}>
            ...
            <div>
              <h1 style={{ fontWeight: 900, fontSize: '1.8rem', lineHeight: 1.2 }}>{t('mock_list_title')}</h1>
      ...
      <div className="container" style={{ padding: '2rem 1.25rem' }}>
        ...
        <div style={{ background: exam.lightColor, border: `1px solid`, borderColor: exam.color + '33', borderRadius: 14, padding: '1rem 1.5rem', marginBottom: '2rem', display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
          {[
            { icon: '✅', text: `+${exam.marking.correct} ${t('mock_list_correct_mark')}` },
            { icon: '❌', text: `${exam.marking.wrong} ${t('mock_list_wrong_mark')}` },
            { icon: '⬜', text: t('mock_list_zero') },
            { icon: '⏱️', text: t('mock_list_timer') },
          ].map(tip => (
            ...
            return (
              <div key={test.id} className="card" style={{ padding: '1.75rem', borderTop: `4px solid ${exam.color}` }}>
                ...
                <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
                  {[
                    { icon: '❓', labelKey: 'mock_stat_questions', val: test.questionIds.length },
                    { icon: '🏅', labelKey: 'mock_stat_total_marks', val: test.totalMarks },
                    { icon: '⏱️', labelKey: 'mock_stat_duration', val: `${test.duration} min` },
                    { icon: '📅', labelKey: 'mock_stat_pattern', val: test.year },
                  ].map(s => (
                    <div key={s.labelKey}>
                      <div style={{ fontSize: '.7rem', color: '#9ca3af', fontWeight: 600 }}>{s.icon} {t(s.labelKey)}</div>
                ...
                {result && (
                  <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 10, padding: '.75rem 1rem', marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ fontSize: '.75rem', color: '#16a34a', fontWeight: 700 }}>{t('mock_last_attempt')}</div>
                    ...
                    <div style={{ textAlign: 'right', fontSize: '.75rem', color: '#16a34a' }}>
                      <div>{result.correct} {t('mock_correct_count')}</div>
                      <div>{result.wrong} {t('mock_wrong_count')}</div>
                    </div>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: '.75rem' }}>
                  <Link to={`/exam/${examId}/mock-test/${test.id}`} style={{ flex: 1 }}>
                    <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }}>
                      {result ? t('mock_retake') : t('mock_start_test')}
                    </button>
                  </Link>
                </div>
              </div>
        ...
        {tests.length === 0 && (
          <div style={{ textAlign: 'center', padding: '4rem', color: '#9ca3af' }}>
            ...
            <div style={{ fontWeight: 700, fontSize: '1.1rem' }}>{t('mock_coming_soon')} {exam.label}</div>
          </div>
        )}
      </div>
    </div>
